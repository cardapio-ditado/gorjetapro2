/*
  # Corrigir trigger para processar requisições internas
  
  1. Changes
    - Corrige a função processar_requisicao_interna para usar quantidade_entregue
*/
...
BEGIN
  ...
  IF NEW.status = 'concluido' AND (OLD.status IS NULL OR OLD.status != 'concluido') THEN
    ...
    LOOP
      ...
      -- Validar: apenas processar se quantidade entregue > 0
      IF COALESCE(item_requisicao.quantidade_entregue, 0) > 0 THEN
        ...
        SELECT 
          ...
          'transferencia',
          item_requisicao.quantidade_entregue,
          COALESCE(ie.custo_medio, 0),
          item_requisicao.quantidade_entregue * COALESCE(ie.custo_medio, 0),
          NEW.data_requisicao,
