/*
  # Criar conta a pagar ao aprovar item ad-hoc sem vínculo

  ## Problema
  
  Itens ad-hoc aprovados sem conta_pagar_id não aparecem na baixa de contas
  porque não existe registro em contas_pagar.
  
  ## Solução
  
  Atualizar api_fin_set_status_item para criar conta a pagar automaticamente
  quando aprovar um item ad-hoc que não tem conta vinculada.
*/
...
-- Corrigir itens ad-hoc já aprovados que não têm conta vinculada
DO $$
DECLARE
  v_item record;
  v_conta_pagar_id uuid;
...
BEGIN
  ...
  LOOP
    ...
    VALUES (
      ...
      v_item.aprovado_por,
      v_item.aprovado_em,
      'Criado automaticamente a partir de item ad-hoc aprovado (correção). ' || COALESCE(v_item.observacao, ''),
      'unica'
    )
    RETURNING id INTO v_conta_pagar_id;
