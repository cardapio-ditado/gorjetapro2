/*
  # Corrigir Duplicação de Movimentações nas Compras

  ## Problema
  A função processar_entrada_compra() não estava preenchendo origem_tipo e origem_id,
  causando violação do índice único idx_unique_movimentacao quando o usuário tenta
  confirmar recebimento múltiplas vezes.

  ## Solução
  1. Adicionar origem_tipo = 'compra' e origem_id = NEW.id
  2. Deletar movimentações existentes dessa compra antes de criar novas (idempotência)
  3. Garantir que mesmo com cliques múltiplos, não haverá duplicação

  ## Comportamento
  - Compra pode ser reconfirmada sem erro
  - Movimentações antigas são substituídas
...
BEGIN
    ...
    IF NEW.status = 'recebido' AND (OLD.status IS NULL OR OLD.status != 'recebido') THEN
        ...
        -- Deletar movimentações antigas dessa compra para evitar duplicação
        ...
        LOOP
            ...
            IF quantidade_efetiva > 0 THEN
                ...
                -- Criar movimentação com origem_tipo e origem_id
