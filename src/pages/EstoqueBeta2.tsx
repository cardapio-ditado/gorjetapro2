...
<div className="b2-shell"><aside className="b2-side"><div className="b2-logo">✦ DITADO POPULAR<small>GORJETA PRO · BETA 2</small></div><nav className="b2-menu">{menu.map(m=><button key={m.v} className="b2-nav" aria-current={view===m.v?'page':undefined} onClick={()=>go(m.v)}><m.icon size={17}/>{m.n}</button>)}</nav><div className="b2-hint">Prévia isolada: não acessa saldos oficiais nem cria movimentações reais.</div></aside>
<main className="b2-main"><div className="b2-head"><span className="b2-tag">● DEMONSTRAÇÃO INTERATIVA</span><span className="b2-muted" style={{fontSize:12}}>DITADO POPULAR · DADOS SIMULADOS</span></div>
