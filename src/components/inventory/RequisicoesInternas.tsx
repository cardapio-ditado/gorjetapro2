...
import {
  ...
  Eye, Download, QrCode, Loader2, ChevronRight, ChevronDown,
  Sparkles, SlidersHorizontal, AlertTriangle, PackageCheck,
} from 'lucide-react';
...
export default function RequisicoesInternas() {
  ...
  const [menuCartaz, setMenuCartaz]                 = useState(false);
  ...
  // ─── Entrega item a item (substitui o antigo "Concluir" tudo-ou-nada) ───────
      ...
      <div className="flex flex-wrap justify-between items-center gap-3">
        ...
        <div className="flex gap-2">
          <div className="relative">
            ...
            >
              <QrCode className="w-4 h-4" /> Gerar Cartaz <ChevronDown className="w-3.5 h-3.5 opacity-60" />
            </button>
            {menuCartaz && (
              <>
                <div className="fixed inset-0 z-30" onClick={() => setMenuCartaz(false)} />
                <div className="absolute right-0 mt-2 w-56 bg-[#0f1020] border border-white/10 rounded-xl shadow-xl z-40 overflow-hidden">
                  {[
                    { label: 'Cartaz do Bar',      href: '/cartaz-requisicao?setor=bar' },
                    { label: 'Cartaz da Cozinha',  href: '/cartaz-requisicao?setor=cozinha' },
                    { label: 'Cartaz geral (antigo)', href: '/cartaz-requisicao' },
                  ].map(op => (
                    <button
                      key={op.href}
                      onClick={() => { window.open(op.href, '_blank'); setMenuCartaz(false); }}
